...
{
	$id=$_GET['id'];
	$stmt = $conn->prepare("SELECT * FROM slider_list WHERE slider_id=".$id);
	$stmt->execute();
...
if(isset($_POST['submit']))
{
	...
	$stmt_update = $conn->prepare('UPDATE slider_list SET name=:name,upload_image=:upload_image,status=:status WHERE slider_id='.$id);		
	$executed=$stmt_update->execute(array(':name' => $name,':upload_image' => $date1,':status' => $status));
	if($executed)
	{
		echo "<script>window.location.href='img_list.php';</script>";
...
<img src="images/slider/<?php echo $row[0]['upload_image']; ?>" style="width:100px;height:100px;">
